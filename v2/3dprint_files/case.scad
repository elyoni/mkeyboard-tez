...
use <stabilizer.scad>
use <standoff.scad>
// use <font\static\ShantellSans-ExtraBold.ttf>
...
module case(switch_layout, mcu_layout, trrs_layout, plate_layout, stab_layout, standoff_layout) {
    ...
    header_text_xloc = invert_layout_flag ? -122 : 97;
    header_text_yloc = invert_layout_flag ? -43 : -18;
    sub_text_xloc = invert_layout_flag ? -109 : 98;
    sub_text_yloc = invert_layout_flag ? -42 : -42.5;
    sub_text_rotate = invert_layout_flag ? 45 : -45;
        ...
        difference() {
            union() {
                ...
                translate([0,0,-height+plate_thickness/2]) difference() {
                    case_shell(height, switch_layout, mcu_layout, trrs_layout, plate_layout, stab_layout);
					translate([header_text_xloc,header_text_yloc,height-plate_thickness*1/2])
                        rotate(a=sub_text_rotate)
                        linear_extrude(2)
                        text("TEZ2", font = "Shantell Sans ExtraBold");
                    translate([sub_text_xloc,sub_text_yloc,height-plate_thickness*1/2])
                        linear_extrude(2)
