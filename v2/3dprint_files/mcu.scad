    ...
    difference() {
        union() {
            ...
            translate([-mcu_socket_width/2,-2,0]) 
                cube([mcu_socket_width,mcu_socket_length,pcb_thickness-1]);
            // Border
...
module mcu_case_cutout() {
    ...
    translate([
        h_unit/2,
        0,
        plate_thickness/2-pcb_plate_spacing+mcu_pcb_thickness/2+mcu_connector_offset-2
    ]) rotate([-90,0,0]) {
        translate([-(mcu_connector_width-2)/2,-4.4,-7]) cube([mcu_connector_width-2,mcu_connector_height,9]);
        hull() {
            for (i=[-1,1]) translate([i*(mcu_connector_width-mcu_connector_height)/2,0,-mcu_connector_length])
                cylinder(h=50, d=mcu_connector_height+2);
        }
    }
